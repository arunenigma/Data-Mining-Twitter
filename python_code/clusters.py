...
# -*- -------------------------------------- -*-
# HEIRARCHIAL CLUSTERING TWITTER DATA (PART 2 OF PROJECT)
...
def hcluster(rows,distance=pearson):
    ...
    # Clusters are initialy just the rows
...
def printclust(clust,labels=None,n=0):
    ...
    #n now print the right and left branches
    if clust.left!=None: printclust(clust.left,labels=labels,n=n+1)
    if clust.right!=None: printclust(clust.right,labels=labels,n=n+1)

def getheight(clust):
    # endpoint? Then the height is just 1
...
def getdepth(clust):
    # The distance of an endopoint is 0.0
...
def drawnode(draw,clust,x,y,scaling,labels):
    if clust.id<0:
        ...
        # If this is an endpoint, draw the item lable
        draw.text((x+5,y-7),labels[clust.id],(0,0,0))

def rotatematrix(data):
    newdata=[]
    for i in range(len(data[0])):
        newrow=[data[j][i] for j in range(50)] # 
